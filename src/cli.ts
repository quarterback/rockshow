...
const USAGE = `oi — Agent AAR Reconciliation Tool

Usage:
  oi reconcile --testimony <file.md> --trace <file.jsonl|file.json> [--out <diff.json>]
...
function main(): void {
  ...
  if (command !== "reconcile") {
    console.error(`oi: unknown command "${command}"\n\n${USAGE}`);
    process.exit(2);
  }
  ...
  if (typeof testimonyPath !== "string" || typeof tracePath !== "string") {
    console.error("oi reconcile: --testimony and --trace are both required.\n");
    console.error(USAGE);
    process.exit(2);
  }
  ...
  try {
    ...
    console.error(`oi: cannot read testimony "${testimonyPath}": ${(e as Error).message}`);
    process.exit(2);
    return;
  }
  try {
    ...
    console.error(`oi: cannot read trace "${tracePath}": ${(e as Error).message}`);
    process.exit(2);
    return;
  }
  ...
  try {
    ...
    console.error(`oi: reconcile failed: ${(e as Error).message}`);
    process.exit(2);
    return;
  }
  ...
  if (typeof args.out === "string") {
    ...
    console.error(`oi: ${diff.status} — wrote ${args.out}`);
  } else {
    console.log(json);
  }
  process.exit(0);
